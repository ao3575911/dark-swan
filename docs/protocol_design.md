...
Dark-Swan is a deterministic, pseudonymous identity protocol built on Ed25519 key pairs and GDk9 symbolic grounding. Every identity is independently verifiable from its public key alone — no central authority, no registration, no seed exposure required for verification.
...
The symbolic ID is a human-memorable 4-letter word derived **only from the public key**. Anyone holding the public key can independently re-derive and verify the symbolic ID. No seed access is needed.

**Known limitation:** The symbolic ID space is 26⁴ = 456,976 unique values. Collision probability rises sharply once thousands of identities are registered. Symbolic IDs are intended as human-readable labels, not globally unique long-term identity keys. See §7 for the planned upgrade.
...
```
symbolic_id (string)  +  epoch_day (int, UTC)
...
## 5. Registry Record Format

The local registry is a JSON file (`registry.json` by default) mapping `symbolic_id → record`.
...
The `signature` covers a canonical JSON serialisation of all other fields (sorted keys, no extra whitespace). Verification requires only the `pubkey` field — no registry lookup.
...
|---|-------|----------|-------------|
| L1 | 4-letter symbolic ID space (26⁴ = 456,976) has high collision risk at scale | High | v0.3: DID / fingerprint upgrade (SHA-256 prefix, full hex or bech32) |
| L2 | SHA-256(seed) used directly as Ed25519 private bytes — no KDF, no salt | High | v0.2.1: document risk; v0.3: optional Argon2id/scrypt KDF |
...
## 8. Planned DID / Fingerprint Upgrade (v0.3)

The current 4-letter symbolic ID will be supplemented by a long-form DID:

```
did:ds:<hex16>
```

where `hex16` is the first 8 bytes of SHA-256(pubkey) as lowercase hex (16 chars). This gives 2⁶⁴ unique values — collision probability negligible for any realistic population. The 4-letter word is retained as the human-memorable short form; the DID becomes the canonical long-form identity key.

Registry records will carry both:
```json
{
  "symbolic_id": "FWEM",
  "did":         "did:ds:a3f9c2b17e04d581",
  ...
}
```
