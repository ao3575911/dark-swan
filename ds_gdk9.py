...
def _sym_energy(ch: str) -> float:
    ...
    if not u.isalpha():
        return 0.0
    pos = ord(u) - ord('A') + 1
    if u in _IDEMP: return float(pos)
    if u in _BIPH:  return math.sin(pos)
    if u in _INVOL: return 1.0 / pos
    if u in _ASYM:  return float(pos + 1)
    return 0.0
...
def dcg_profile(word: str) -> Optional[Dict]:
    ...
    try:
        from gdk9.dcg import get_dcg, homotopy_equivalent
        g = get_dcg()
